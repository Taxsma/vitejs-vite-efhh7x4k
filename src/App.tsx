...
const isSameDay = (date1: Date | string, date2: Date | string) =>
  getDateString(new Date(date1)) === getDateString(new Date(date2));
...
function MainApp() {
  const [activeView, setActiveView] = useState<
    'home' | 'profile' | 'library' | 'create_food'
  >('home');
  ...
  const [selectedMeal, setSelectedMeal] = useState(getDefaultMeal()); // 自动获取当前时间的默认餐次

  const [customFoods, setCustomFoods] = useState<FoodItem[]>(defaultFoods);
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedFoodForAdd, setSelectedFoodForAdd] = useState<FoodItem | null>(
    null
  );
  ...
  useEffect(() => {
    try {
      ...
      if (savedHistory) {
        const p = JSON.parse(savedHistory);
        if (Array.isArray(p)) setAllHistory(p);
      }
      if (savedGoal) {
      ...
      if (savedWater) {
        const w = JSON.parse(savedWater);
        if (w && typeof w === 'object') setWaterLog(w);
      }
      if (savedFoods) {
        const f = JSON.parse(savedFoods);
        if (Array.isArray(f) && f.length > 0) setCustomFoods(f);
      }
      if (savedProfile) {
  ...
  }, [allHistory, goal, waterLog, profile, customFoods]);

  const safeHistory = Array.isArray(allHistory) ? allHistory : [];
  const currentRecords = safeHistory
    .filter(
      (item) => item && item.timestamp && isSameDay(currentDate, item.timestamp)
    )
    .sort(
      (a, b) =>
  ...
  const consumed = currentRecords.reduce(
    (sum, item) => sum + (item.calories || 0),
    0
  );
  const consumedProtein = currentRecords.reduce(
    (sum, item) => sum + (item.protein || 0),
    0
  );
  const consumedCarbs = currentRecords.reduce(
    (sum, item) => sum + (item.carbs || 0),
    0
  );
  const consumedFat = currentRecords.reduce(
  ...
  const goalCarbs = Math.round((goal * 0.4) / 4) || 1;
  const goalProtein = Math.round((goal * 0.3) / 4) || 1;
  const goalFat = Math.round((goal * 0.3) / 9) || 1;
  ...
  const addWater = () =>
    setWaterLog((prev) => ({
      ...prev,
      [currentDateKey]: Math.min((prev[currentDateKey] || 0) + 1, 8),
    }));
  const removeWater = () =>
    setWaterLog((prev) => ({
      ...prev,
      [currentDateKey]: Math.max((prev[currentDateKey] || 0) - 1, 0),
    }));
  const deleteRecord = (id: number) =>
    setAllHistory(safeHistory.filter((item) => item.id !== id));
  ...
  const safeCustomFoods = Array.isArray(customFoods)
    ? customFoods
    : defaultFoods;
  ...
  const confirmAddRecord = () => {
    ...
    const newItem: RecordItem = {
      ...
      amount: amount,
      timestamp: isToday(currentDate)
        ? new Date().toISOString()
        : new Date(new Date(currentDate).setHours(12, 0, 0, 0)).toISOString(),
    };
  ...
  const copyYesterdayMeal = (mealId: string) => {
    ...
    const yesterdayRecords = safeHistory.filter(
      (r) => isSameDay(r.timestamp, yesterday) && r.mealType === mealId
    );
  ...
  const handleAddNewCustomFood = () => {
    ...
    const createdFood: FoodItem = {
      ...
      name: newFood.name,
      unit: newFood.unit || '1份',
      iconStr: newFood.iconStr,
  ...
  const filteredFoods = safeCustomFoods.filter(
    (f) =>
      f &&
      f.name &&
    ...
    return {
      ...
      bmiColor:
        bmi < 18.5
          ...
          : 'text-red-500',
      bmiStatus:
        bmi < 18.5 ? '偏瘦' : bmi < 24 ? '标准' : bmi < 28 ? '超重' : '肥胖',
    };
  };
      ...
      <MobileContainer>
        ...
        <div className="p-5 overflow-y-auto no-scrollbar pb-20">
          <div className="bg-gray-100 text-gray-900 p-5 rounded-xl border-4 border-gray-300 shadow-xl mb-6 font-sans">
            <h1 className="text-3xl font-black border-b-8 border-gray-900 pb-2 mb-2 tracking-tighter uppercase">
              Nutrition Facts
            </h1>
            ...
            <div className="flex justify-between items-center border-b-4 border-gray-900 pb-2 mb-2">
              ...
              <input
                ...
                className="bg-transparent text-right font-bold outline-none placeholder-gray-400 w-3/5"
              />
            </div>
          ...
          <div className="mb-6">
            ...
            <div className="flex flex-wrap gap-3">
              {Object.keys(iconMap).map((key) => (
                <button
                  key={key}
                  onClick={() => setNewFood({ ...newFood, iconStr: key })}
                  className={`p-3 rounded-xl transition-all text-2xl ${
                    newFood.iconStr === key
                      ? 'bg-emerald-500/20 border border-emerald-500/50 scale-110 shadow-lg'
          ...
          <button
            onClick={handleAddNewCustomFood}
            disabled={!newFood.name || !newFood.cal}
            className="w-full bg-emerald-500 hover:bg-emerald-400 disabled:opacity-30 disabled:cursor-not-allowed text-gray-950 font-black py-4 rounded-2xl transition-all flex justify-center items-center gap-2 shadow-[0_0_20px_rgba(16,185,129,0.3)]"
          >
      ...
      <MobileContainer>
        <div className="p-4 border-b border-gray-800 bg-gray-950/80 backdrop-blur-md sticky top-0 z-20">
          <div className="flex items-center gap-3 mb-4">
            ...
            <button
              onClick={() => setActiveView('create_food')}
              className="flex items-center gap-1 text-sm font-bold text-emerald-400 bg-emerald-500/10 px-3 py-1.5 rounded-full"
            >
          ...
          <div className="relative">
            ...
            <input
              type="text"
              placeholder="搜索食物 (支持拼音/汉字)..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full bg-gray-900 border border-gray-800 text-white pl-10 pr-4 py-3 rounded-xl outline-none focus:border-emerald-500 transition-colors placeholder-gray-600"
            />
            {searchQuery && (
              <button
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar p-4 pb-32">
          {filteredFoods.length === 0 ? (
            <div className="text-center mt-10 text-gray-500 flex flex-col items-center">
              ...
              <button
                onClick={() => setActiveView('create_food')}
                className="mt-4 text-emerald-400 underline underline-offset-4"
              >
                去手动新建一个？
              </button>
            </div>
            ...
            <div className="space-y-3">
              {filteredFoods.map((food) => (
                <div
                  ...
                  className="flex items-center justify-between bg-gray-900 hover:bg-gray-800 p-4 rounded-2xl border border-gray-800/80 cursor-pointer transition-colors active:scale-95"
                >
                  <div className="flex items-center gap-4">
                    <span className="text-2xl">
                      {iconMap[food.iconStr] || '🍽️'}
                    </span>
                    <div>
                      ...
                      <div className="text-xs text-gray-500 mt-1 flex gap-2">
                        <span>碳 {food.c}g</span>
                        <span>蛋 {food.p}g</span>
                        <span>脂 {food.f}g</span>
                      </div>
                    </div>
                  </div>
          ...
          <div className="absolute inset-x-0 bottom-0 bg-gray-900 border-t border-gray-800 rounded-t-3xl p-6 shadow-[0_-10px_40px_rgba(0,0,0,0.5)] z-30 animate-in slide-in-from-bottom-10 duration-200">
            <div className="flex justify-between items-start mb-6">
              <div>
                <h3 className="text-xl font-bold text-white flex items-center gap-2">
                  {selectedFoodForAdd.name}
                </h3>
                <p className="text-gray-400 text-sm mt-1">
                  每 {selectedFoodForAdd.unit} 含 {selectedFoodForAdd.cal} kcal
                </p>
              </div>
              <button
            ...
            <div className="flex items-center justify-between mb-6 bg-gray-950 p-4 rounded-2xl border border-gray-800">
              <span className="text-gray-400 font-bold text-sm">
                食用份数 (乘以 {selectedFoodForAdd.unit})
              </span>
              <div className="flex items-center gap-3">
                <button
                  onClick={() =>
                    setAddAmount(
                      Math.max(0.1, parseFloat(addAmount) - 0.5).toString()
                    )
                  }
                  className="w-8 h-8 rounded-full bg-gray-800 flex items-center justify-center text-emerald-400"
                >
                  -
                </button>
                <input
                ...
                <button
                  onClick={() =>
                    setAddAmount((parseFloat(addAmount) + 0.5).toString())
                  }
            ...
            <div className="flex gap-2 mb-6">
              {mealTypes.map((meal) => (
                <button
                  ...
                  className={`flex-1 py-2.5 flex items-center justify-center gap-1.5 text-sm font-bold rounded-xl transition-all border ${
                    selectedMeal === meal.id
                      ? 'bg-emerald-500 text-gray-950 border-emerald-500 shadow-lg shadow-emerald-500/20'
            ...
            <button
              ...
              className="w-full bg-emerald-500 hover:bg-emerald-400 text-gray-950 font-black py-4 rounded-2xl transition-transform active:scale-95 flex items-center justify-center gap-2 text-lg"
            >
              <Icon name="PlusCircle" size={22} /> 确认打卡 (
              {(selectedFoodForAdd.cal * (parseFloat(addAmount) || 1)).toFixed(
                0
              )}{' '}
              kcal)
            </button>
        ...
        <div className="p-4 overflow-y-auto no-scrollbar pb-10">
            ...
            <div className="flex gap-4">
              ...
              <div className="flex-1">
                ...
                <input
                  ...
                  className="w-full bg-gray-800 text-white px-3 py-2 rounded-xl text-center outline-none"
                />
              </div>
            </div>
            <div className="flex gap-4">
              ...
              <div className="flex-1">
                ...
                <input
                  ...
                  className="w-full bg-gray-800 text-emerald-400 font-bold px-3 py-2 rounded-xl text-center outline-none"
                />
              </div>
            </div>
            <div>
              <label className="block text-xs text-gray-500 mb-2 mt-2">
                活动水平
              </label>
              <div className="space-y-2">
                {[
                  ...
                  <div
                    key={a.val}
                    onClick={() =>
                      setProfile({ ...profile, activityLevel: a.val })
                    }
                    className={`p-3 rounded-xl border flex justify-between ${
                      profile.activityLevel === a.val
          ...
          <div className="bg-gray-900 rounded-2xl p-5 mb-6 border border-gray-800 shadow-lg relative overflow-hidden">
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div className="bg-gray-800 p-4 rounded-xl">
                ...
                <div className={`text-2xl font-black ${metrics.bmiColor}`}>
                  {metrics.bmi}
                </div>
              </div>
              <div className="bg-gray-800 p-4 rounded-xl">
            ...
            <div className="bg-gray-800 p-4 rounded-xl flex justify-between items-center">
              <div>
                <div className="text-xs text-gray-500 mb-1">TDEE (总消耗)</div>
          ...
          <div className="bg-gray-900 rounded-2xl p-5 border border-gray-800 shadow-lg">
            ...
            <div className="space-y-3">
              <button
                onClick={() => {
                  setGoal(metrics.tdee - 500);
                  setActiveView('home');
                }}
                className="w-full flex justify-between p-4 rounded-xl bg-gray-800 text-left"
              >
                <span className="text-sm font-bold text-blue-400">
                  🔥 减脂 (-500)
                </span>
                <span className="text-lg font-black">{metrics.tdee - 500}</span>
              </button>
              <button
                onClick={() => {
                  setGoal(metrics.tdee);
                  setActiveView('home');
                }}
                className="w-full flex justify-between p-4 rounded-xl bg-gray-800 text-left"
              >
                ...
                <span className="text-lg font-black">{metrics.tdee}</span>
              </button>
              <button
                onClick={() => {
                  setGoal(metrics.tdee + 300);
                  setActiveView('home');
                }}
                className="w-full flex justify-between p-4 rounded-xl bg-gray-800 text-left"
              >
                ...
                <span className="text-lg font-black">{metrics.tdee + 300}</span>
              </button>
            </div>
          </div>
        </div>
    ...
    <MobileContainer>
      <div className="bg-gray-900 border-b border-gray-800 px-4 pt-6 pb-2 z-20 rounded-b-3xl relative">
        <div className="flex justify-between items-center mb-4">
          ...
          >
            <div className="bg-gray-800 p-2 rounded-xl text-emerald-400 group-hover:bg-gray-700">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500 font-medium">
                {isSameDay(currentDate, new Date()) ? '今天' : '查看历史'}
              </p>
            </div>
          </div>
          <button
            onClick={() => setActiveView('profile')}
            className="p-2 text-emerald-400 bg-gray-800 rounded-xl relative"
          >
        ...
        <div
          className={`transition-all duration-500 ease-in-out overflow-hidden ${
            isCalendarExpanded ? 'max-h-[22rem]' : 'max-h-12'
          }`}
        >
          <div
            className={`grid grid-cols-7 gap-1 ${
              isCalendarExpanded ? 'hidden' : 'block'
            }`}
          >
            {getWeekDays(currentDate).map((date, idx) => {
              ...
              return (
                ...
                >
                  <div
                    className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center text-sm font-bold ${
                      selected
                  ...
                  {isToday(date) && !selected && (
                    <div className="w-1 h-1 bg-emerald-500 rounded-full mt-1 absolute bottom-0"></div>
                  )}
                </div>
              );
            })}
          ...
          <div className={isCalendarExpanded ? 'block' : 'hidden'}>
            <div className="grid grid-cols-7 gap-y-1 gap-x-1 pb-1">
              {getMonthDays(
                ...
                const selected = isSameDay(date, currentDate),
                  isCurMonth = date.getMonth() === calendarViewDate.getMonth();
                return (
                  <div
                    key={idx}
                    onClick={() => {
      ...
      <div className="flex-1 overflow-y-auto no-scrollbar p-4 pb-10">
        ...
        <div className="bg-gray-900 rounded-2xl p-5 mb-5 shadow-lg border border-gray-800">
            ...
            {isEditingGoal ? (
              <div className="flex items-center gap-2">
                <input
                  ...
                  className="w-16 bg-gray-800 text-sm text-center text-emerald-400 rounded outline-none py-1"
                />
                <button onClick={saveGoal} className="text-emerald-500">
                  <Icon name="Check" size={16} />
                </button>
              </div>
            ) : (
              <div
                className="flex items-center gap-1 group cursor-pointer bg-gray-800 px-2 py-1 rounded-md"
                onClick={() => {
          ...
          <div className="mb-4 text-5xl font-black tracking-tighter text-emerald-400">
            ...
            <div>
              ...
              <div className="w-full bg-gray-800 rounded-full h-1">
                <div
                  ...
                  style={{
                    width: `${Math.min((consumedFat / goalFat) * 100, 100)}%`,
                  }}
                ></div>
              </div>
            </div>
        ...
        <div className="bg-gray-900 rounded-2xl p-4 mb-6 border border-gray-800 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-gray-300">饮水打卡</span>
          </div>
          <div className="flex gap-1">
            {[...Array(8)].map((_, i) => (
              <button
                key={i}
                onClick={i < currentWater ? removeWater : addWater}
                className={`w-5 h-7 rounded-lg transition-all ${
                  i < currentWater
              ...
              >
                <div
                  className={`w-full h-full rounded-lg opacity-50 ${
                    i < currentWater ? 'bg-cyan-400' : 'bg-transparent'
                  }`}
                ></div>
              </button>
            ))}
        ...
        <div className="bg-gray-900 rounded-2xl p-4 border border-gray-800 shadow-lg mb-6 relative overflow-hidden">
          {(() => {
            ...
            const mealRecords = currentRecords.filter(
              (r) => r.mealType === selectedMeal
            );
            ...
            yesterday.setDate(yesterday.getDate() - 1);
            const yesterdayRecords = safeHistory.filter(
              (r) => isSameDay(r.timestamp, yesterday) && r.mealType === meal.id
            );
                  ...
                  {mealRecords.length > 0 ? (
                    <div className="space-y-2">
                      {mealRecords.map((item) => (
                        <div
                          key={item.id}
                          className="flex justify-between items-center bg-gray-950 p-3.5 rounded-xl border border-gray-800 group relative"
                        >
                          <div className="flex-1">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <button
                              ...
                              className="opacity-0 group-hover:opacity-100 text-red-500 p-2 hover:bg-red-500/20 rounded-lg transition-opacity"
                            >
                    ...
                    <div className="flex flex-col items-center justify-center h-full pt-4">
                      <button
                        ...
                        className="w-full py-4 border border-dashed border-emerald-500/30 rounded-xl text-emerald-400 text-sm font-medium bg-emerald-500/5 hover:bg-emerald-500/10 flex justify-center items-center gap-2 transition-colors"
                      >
                        <Icon name="Copy" size={18} /> 一键复制昨天{meal.name} (
                        {yesterdayRecords.reduce((s, r) => s + r.calories, 0)}{' '}
                        kcal)
                      </button>
                    </div>
                  ) : (
                    <div className="flex flex-col items-center justify-center h-full pt-6 pb-2 text-center text-sm text-gray-600">
                      <div className="opacity-20 mb-3 text-4xl">
                        {meal.icon}
                      </div>
                      还没有记录{meal.name}哦 🍽️
                    </div>
